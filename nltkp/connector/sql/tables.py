"""The module provides a function to retrieve information about tables in an SQL database using SQLAlchemy."""
...
def get_table_info_from_sql_database(  # noqa: C901, PLR0913
    ...
    *,
    ...
    ----
        ...
        schema (str | None): Schema to reflect.
        table_names (list[str] | None): List of specific table names to retrieve information for.
        view_support (bool): Whether to include view names in the info.
        custom_table_info (dict[str, str] | None): Custom info for specific tables.
        max_string_length (int): Maximum length for string columns.
        indexes_in_table_info (bool): Whether to include indexes in the info.
        sample_rows_in_table_info (int): Number of sample rows to include in the info.
    ...
    """
    inspector: Any = inspect(subject=engine)
    all_tables: set[str] = set(
        inspector.get_table_names(schema=schema) + (inspector.get_view_names(schema=schema) if view_support else []),
    )
    ...
    meta_tables: list[Table] = [
        ...
        if tbl.name in usable_table_names and not (engine.dialect.name == "sqlite" and tbl.name.startswith("sqlite_"))
    ]
    ...
    def _get_sample_rows(table: Table) -> str:
        ...
        try:
            with engine.connect() as connection:
                ...
                sample_rows: list[list[str]] = [[str(i)[:max_string_length] for i in row] for row in sample_rows_result]
